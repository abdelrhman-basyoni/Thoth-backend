...
func RegisterBlogRoutes(e *echo.Echo, db *gorm.DB) {
	...
	BlogGroup.POST("/create", controller.HandleCreate, middlewares.RoleAuth([]string{types.Roles.Admin}))
	BlogGroup.POST("/publish/:id", controller.HandlePublish, middlewares.RoleAuth([]string{types.Roles.Admin}))
